...
export default function Page({
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      // Verifica também chaves específicas do teste A/B
      const storedAbResumeTime = Number(localStorage.getItem('ab-' + videoId + '-resume') || 0);
      const storedAbTime = Number(localStorage.getItem('ab-' + videoId) || 0);
      const storedAbTestResumeTime = Number(localStorage.getItem('ab-test-' + videoId + '-resume') || 0);
      const storedAbTestTime = Number(localStorage.getItem('ab-test-' + videoId) || 0);
      
      // Pega o maior valor entre todas as chaves
      const storedVideoTime = Math.max(
        storedResumeTime,
        storedPlainTime,
        storedAbResumeTime,
        storedAbTime,
        storedAbTestResumeTime,
        storedAbTestTime
      );
